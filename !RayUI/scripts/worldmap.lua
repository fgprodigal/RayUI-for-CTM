...
hooksecurefunc("EncounterJournal_AddMapButtons",function()
	...
	if WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE then
		...
		while _G["EJMapButton"..index] do
			_G["EJMapButton"..index]:SetSize(EJbuttonWidth * C["modules"].ejbuttonscale, EJbuttonHeight * C["modules"].ejbuttonscale)
			_G["EJMapButton"..index].bgImage:SetSize(EJbuttonImageWidth * C["modules"].ejbuttonscale,EJbuttonImageHeigth * C["modules"].ejbuttonscale)
			index = index + 1
		end
	else
		local index = 1
...
if C["modules"].worldmapscale==1 then C["modules"].worldmapscale = 0.99 end -- dirty hack to prevent 'division by zero'!
...
w:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_ENTERING_WORLD" then
		...
		WORLDMAP_RATIO_MINI = C["modules"].worldmapscale
		WORLDMAP_WINDOWED_SIZE = C["modules"].worldmapscale 
		WORLDMAP_SETTINGS.size = C["modules"].worldmapscale 
		...
		for i = 1,40 do
			...
			ri:SetWidth(C["modules"].partymemberscale)
			ri:SetHeight(C["modules"].partymemberscale)
		end
		for i = 1,4 do
...
local function SmallSkin()
	...
	if C["modules"].worldmaplock then
		WorldMapDetailFrame:ClearAllPoints()
		WorldMapDetailFrame:SetPoint(unpack(mpos))
	end
	WorldMapFrame.scale = C["modules"].worldmapscale
	WorldMapDetailFrame:SetScale(C["modules"].worldmapscale)
	WorldMapButton:SetScale(C["modules"].worldmapscale)
	WorldMapFrameAreaFrame:SetScale(C["modules"].worldmapscale)
	WorldMapTitleButton:Show()
	WorldMapFrameMiniBorderLeft:Hide()
	WorldMapFrameMiniBorderRight:Hide()
	WorldMapPOIFrame.ratio = C["modules"].worldmapscale
	...
	WorldMapFrameSizeUpButton:SetScale(C["modules"].worldmapscale)
	...
	WorldMapFrameCloseButton:SetScale(C["modules"].worldmapscale)
	...
	WorldMapQuestShowObjectives:SetScale(C["modules"].worldmapscale)
	WorldMapQuestShowObjectives:SetScale(C["modules"].worldmapscale)
	WorldMapShowDigSites:SetScale(C["modules"].worldmapscale)
	WorldMapTrackQuest:SetScale(C["modules"].worldmapscale)
	WorldMapLevelDownButton:SetScale(C["modules"].worldmapscale)
	WorldMapLevelUpButton:SetScale(C["modules"].worldmapscale)
