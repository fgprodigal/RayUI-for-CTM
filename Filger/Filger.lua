		...
		if (not bar) then
			...
			if (self.Mode == "ICON") then
				...
				if (bar.count) then
					...
					bar.count:SetFont(C["media"].font, 14, "OUTLINE")
				...
				if (bar.background)then
					...
					bar.background:SetTexture(C["media"].normal)
	...
	if ((unit == "target" or unit == "player" or unit == "pet" or unit == "focus") or event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_ENTERING_WORLD" or event == "SPELL_UPDATE_COOLDOWN") then
		...
		for i=1, #Filger_Spells[class][id], 1 do
			...
			if ((name and (data.caster ~= 1 and (caster == data.caster or data.caster == "all") or MyUnits[caster])) or ((enabled or 0) > 0 and (duration or 0) > 1.5)) then
				table.insert(active[id], { data = data, icon = icon, count = count, duration = duration, expirationTime = expirationTime or start, debuffType = debuffType })
			end
		end
		Update(self)
	end
...
local AnchorLoader = CreateFrame("Frame")
AnchorLoader:RegisterEvent("PLAYER_ENTERING_WORLD")
AnchorLoader:SetScript("OnEvent", function(self,event)
	self:UnregisterEvent("PLAYER_ENTERING_WORLD")		
	for i = 1, #Filger_Spells[class], 1 do
		data = Filger_Spells[class][i]
		_G["FilgerAnchor"..i]:Point(unpack(data.setPoint))
