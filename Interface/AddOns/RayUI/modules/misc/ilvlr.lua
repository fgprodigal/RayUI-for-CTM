...
local function LoadFunc()
	...
	local function fetchSocketCount(slotName)
	...
	local function fetchGem(slotName)
		local inventoryID = GetInventorySlotInfo(slotName)
		local itemLink = GetInventoryItemLink("player", inventoryID)
	...
	local function fetchBaseSocket(slotName)
		local inventoryID = GetInventorySlotInfo(slotName)
		local itemLink = GetInventoryItemLink("player", inventoryID)
	...
	local function fetchChant(slotName)
		local inventoryID = GetInventorySlotInfo(slotName)
		local itemLink = GetInventoryItemLink("player", inventoryID)
	...
	local function fetchSubclass(slotName)
		local slotId, texture, checkRelic = GetInventorySlotInfo(slotName)
		local itemId = GetInventoryItemID("player", slotId)
		if itemId then
			local name, link, quality, iLevel, reqLevel, class, subclass, maxStack, equipSlot, texture, vendorPrice = GetItemInfo(itemId)
	...
	local function makeIlvl(frame, slot)
		...
		if not iLvl then
			...
			isValid = iLvlText:SetFont(R["media"].pxfont, R.mult*10, "OUTLINE,MONOCHROME")
	...
	local function makeMod(frame, slot)
		...
		if not iMod then
			...
			isValid = iModText:SetFont(R["media"].pxfont, R.mult*10, "OUTLINE,MONOCHROME")
	...
	local function iLvLrOnItemUpdate()
	...
	local function iLvLrOnModUpdate()
		for k ,v in pairs(slotDB) do
			local iLevel = fetchIlvl(v)
			if iLevel then
				if v == "ShirtSlot" or v == "TabardSlot" then
	...
	local function iLvLrOnEvent(self, event)
	...
	local iLvLrFrame = CreateFrame("Frame", "iLvLrFrame", UIParent)
