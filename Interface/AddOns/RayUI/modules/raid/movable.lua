...
local saveDefaultPosition = function(anchor)
    local style, identifier = "RayUIRaid", anchor:GetName()
...
do
    ...
    setframe = function(frame)
        frame:SetHeight(RA.db.height)
        frame:SetWidth(RA.db.width)
...
function RA:Anchors()
    local raidframe = CreateFrame("Frame", "oUF_RayUIRaidRaidFrame", UIParent)
    setframe(raidframe)
    raidframe.ident = "Raid"
    raidframe.name:SetText("Raid")
    raidframe:SetPoint("LEFT", UIParent, "LEFT", 8, 0)
    anchorpool["oUF_RayUIRaidRaidFrame"] = raidframe

    local petframe = CreateFrame("Frame", "oUF_RayUIRaidPetFrame", UIParent)
    setframe(petframe)
    petframe.ident = "Pet"
    petframe.name:SetText("Pet")
    petframe:SetPoint("LEFT", UIParent, "LEFT", 250, 0)
    anchorpool["oUF_RayUIRaidPetFrame"] = petframe

    local mtframe = CreateFrame("Frame", "oUF_RayUIRaidMTFrame", UIParent)
    setframe(mtframe)
    mtframe.ident = "MT"
    mtframe.name:SetText("MT")
    mtframe:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 8, -60)
    anchorpool["oUF_RayUIRaidMTFrame"] = mtframe
