...
Stat:SetScript("OnEvent", function(self)
	...
	self:SetValue(value)
	TopInfoBar4.Text:SetText(DURABILITY..": "..value.."%")
	if value<10 then
		self:SetStatusBarColor(unpack(R.InfoBarStatusColor[3]))
	elseif value<30 then
		self:SetStatusBarColor(unpack(R.InfoBarStatusColor[2]))
	else
		self:SetStatusBarColor(unpack(R.InfoBarStatusColor[1]))
	end
