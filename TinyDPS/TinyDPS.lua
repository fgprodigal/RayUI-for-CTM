R, C = unpack(RayUI) -- Import: E - functions, constants, variables; C - config; L - locales
...
--[[-----------------------------------------------------------------------------------------------------------------------------
	...
	* Skinned to match Tukui, by Dajova
...
---------------------------------------------------------------------------------------------------------------------------------
	...
	elseif GetLocale() == 'zhCN' then
		...
		tdpsL.spacing = '间距'
		tdpsL.maximum = '最多显示'
		tdpsL.oneYourself = '1 (自身)'
		tdpsL.five = '5'
		tdpsL.ten = '10'
		tdpsL.fifteen = '15'
		tdpsL.twenty = '20'
		tdpsL.unlimited = '? (显示所有)'
		...
		tdpsL.swapBarTextColor = '互换 计量条/文字 颜色'
		
		tdpsL.history = '      %i  历史战斗记录'
		tdpsL.keepOnlyBossFights = '仅保留BOSS战斗记录'
...
---------------------------------------------------------------------------------------------------------------------------------
	...
	local font, fonts = C["media"].font, 12
	...
	local function initialiseSavedVariables()
		...
		tdps = {
			...
			layout = 12,
			showRank = true,
			onlyBossSegments = true,
			showMinimapButton = false,
			spacing = 1, barHeight = 18,
			bar = {.9, .9, .9, 1}, barbackdrop = {1, 1, 1, .05}, border = {unpack(C["media"].bordercolor)}, backdrop = {unpack(C["media"].backdropcolor)},
		}
		...
		else tdpsFont = {name = font, size = fonts, outline = 'Outline', shadow = 0} end
...
---------------------------------------------------------------------------------------------------------------------------------
	...
	tdpsAnchor:SetPoint('TOPRIGHT', Minimap, 'TOPLEFT', -5, 2)
	tdpsAnchor:SetFrameStrata('BACKGROUND')
	-- tdpsAnchor:CreateShadow("Background")
	...
	tdpsFrame:SetPoint('TOPRIGHT', tdpsAnchor, 'TOPRIGHT', 0, 0)
	tdpsFrame:SetFrameStrata('MEDIUM')
	tdpsFrame:SetFrameLevel(1)
	tdpsFrame:CreateShadow("Background")
...
---------------------------------------------------------------------------------------------------------------------------------
	...
	local textLayout = { -- bits: 8 = dps, 4 = percentage, 2 = amount, 1 = short format. Example: 13 = 1101 = percentage and dps (short)
		...
		[12] = function(i, n, t) bar[i].fontStringRight:SetFormattedText('(%i, %i%%)', n/t, n/tdpsFight[tdpsF][tdpsV]*100) end,
		[13] = function(i, n, t) bar[i].fontStringRight:SetFormattedText('%i%% %s', n/tdpsFight[tdpsF][tdpsV]*100, short(n/t)) end,
		[14] = function(i, n, t) bar[i].fontStringRight:SetFormattedText('%i (%i, %i%%)', n, n/t, n/tdpsFight[tdpsF][tdpsV]*100) end,
		[15] = function(i, n, t) bar[i].fontStringRight:SetFormattedText('%s (%s, %i%%)', short(n), short(n/t), n/tdpsFight[tdpsF][tdpsV]*100) end
	...
	local function newBar(g)
		...
		dummybar:SetBackdrop({bgFile = C["media"].normal, edgeFile = C["media"].blank, tile = false, tileSize = R.mult, edgeSize = R.mult, insets = { left = 0, right = 0, top = 0, bottom = 0}})
		dummybar:SetStatusBarTexture(C["media"].normal)
...
---------------------------------------------------------------------------------------------------------------------------------
	...
	local function tdpsCombatEvent(self, event, ...)
		...
		local arg1, arg2, _, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13 = ...
