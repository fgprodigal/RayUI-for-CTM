...
GameTooltipStatusBar:SetScript("OnValueChanged", function(self, value)
    ...
    if unit then
        ...
        if not self.text then
			...
			if cfg.outline=="NONE" then self.text:SetShadowOffset(R.mult, -R.mult) end
        end
        self.text:Show()
    ...
    if frame.NumLines then
        for index=1, frame:NumLines() do
            if index == 1 then
				...
				if cfg.outline=="NONE" then _G[frame:GetName()..'TextLeft'..index]:SetShadowOffset(R.mult, -R.mult) end
            else
